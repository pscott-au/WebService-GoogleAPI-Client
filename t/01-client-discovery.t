...
subtest 'WebService::GoogleAPI::Client::Discovery class properties'  => sub {
    ok( ref WebService::GoogleAPI::Client::Discovery->new->ua eq 'WebService::GoogleAPI::Client::UserAgent', 'ua property (WebService::GoogleAPI::Client::Discovery->new->ua) is of type WebService::GoogleAPI::Client::UserAgent' );
    ok( ref(WebService::GoogleAPI::Client::Discovery->new->chi) =~ /^CHI::Driver/xm, 'chi property (WebService::GoogleAPI::Client::Discovery->new->chi) is of sub-type CHI::Driver::' );
...
ok( WebService::GoogleAPI::Client::Discovery->new->list_of_available_google_api_ids() =~ /gmail/xm, 'WebService::GoogleAPI::Client::Discovery->new->list_of_available_google_api_ids()' );
...
ok ( ref ( WebService::GoogleAPI::Client::Discovery->new->augment_discover_all_with_unlisted_experimental_api() ) eq 'HASH', ' WebService::GoogleAPI::Client::Discovery->new->augment_discover_all_with_unlisted_experimental_api() returns HASHREF');
ok ( length(  WebService::GoogleAPI::Client::Discovery->new->supported_as_text ) > 100, 'WebService::GoogleAPI::Client::Discovery->new->supported_as_text() returns string > 100 chars' );
...
#$ENV{CHI_FILE_PATH} = $ENV{TMPDIR};
        ...
        ok ( my $chi = CHI->new(
            ...
            #depth          => 3,
            max_key_length => 512
        ), 'CREATE testing CHI File using path from $ENV{CHI_FILE_PATH}'); ## so don't do network requests unless we have taken the extra step and are more likely to know the impact
        ok(  $gapi = WebService::GoogleAPI::Client->new( debug => $DEBUG, chi=>$chi, gapi_json=> $default_file ), 'Creating test session instance of WebService::GoogleAPI::Client using customised CHI');
        ...
        ok( join(',', sort keys ( %{WebService::GoogleAPI::Client::Discovery->new->discover_all() }) )  eq 'discoveryVersion,items,kind', 'Client->discover_all returns HASHREF with keys of discoveryVersion,items,kind');
      ...
      #  ok ( scalar( grep {/^gmail$/xmg} $gapi->list_of_available_google_api_ids() ) ==1, 'WebService::GoogleAPI::Client->list_of_available_google_api_ids() returned ARRAY includes "gmail"');
        ...
        ok ( scalar( $gapi->methods_available_for_google_api_id('gmail') ) > 10, 'WebService::GoogleAPI::Client->methods_available_for_google_api_id("gmail") returns count of more than 10 '  );

        ok( my $api_detail = $gapi->discovery->extract_method_discovery_detail_from_api_spec('gmail.users.settings'), "WebService::GoogleAPI::Client->extract_method_discovery_detail_from_api_spec('gmail.users.settings') returns something");
        ok( keys %{ $gapi->extract_method_discovery_detail_from_api_spec('gmail.us.dummy-illegal-param-test')} ==0, "WebService::GoogleAPI::Client->extract_method_discovery_detail_from_api_spec('gmail.us.dummy') returns empty hashref");
        ok( keys %{ $gapi->extract_method_discovery_detail_from_api_spec('not-a-google-service.list')} ==0, "WebService::GoogleAPI::Client->extract_method_discovery_detail_from_api_spec('not-a-google-service.list') returns empty hashref");

        ok( scalar( [$gapi->get_scopes_as_array()] ) > 0,'more than 0 scopes returned from config');
        ok( $gapi->has_scope_to_access_api_endpoint("gmail.users.messages.send") =~ /^0|1$/xmg , 'has_scope_to_access_api_endpoint("gmail.users.messages.send") returns either 0 or 1'); # 

        cmp_ok( my $permission = $gapi->has_scope_to_access_api_endpoint("sheets.spreadsheets.get"), '>=', 0  , 'has_scope_to_access_api_endpoint("sheets.spreadsheets.get") returns >= 0 '); # 
        ...
        if ( $permission == 0 )
        {
             ...
             ok( $gapi->api_query( {api_endpoint_id => 'sheets.spreadsheets.get'} )->code() eq '418', "Access to service without scope permission returns expected HTTP 418 error - I'm a teapot");
        }

        cmp_ok( $permission = $gapi->has_scope_to_access_api_endpoint("gmail.users.messages.send"), '>=', 0  , 'has_scope_to_access_api_endpoint("gmail.users.messages.send") returns >= 0 '); #
