...
has 'credentials' => ( is => 'rw', default => sub { WebService::GoogleAPI::Client::Credentials->instance }, handles => [qw/access_token auth_storage get_scopes_as_array user /], lazy => 1 );
...
sub refresh_access_token
{
  ...
  if (     ( !defined $credentials->{ client_id } )       ## could this be caught somewhere earlier than here?
     ...
     )
  {
    ...
    'you only asked for an access token.' if !defined $credentials->{ refresh_token };
