...
has 'ua'        => ( handles => [qw/access_token auth_storage  do_autorefresh get_scopes_as_array user /],
  is      => 'ro',
...
has 'discovery' => ( handles => [qw/ get_method_meta discover_all extract_method_discovery_detail_from_api_spec get_api_discovery_for_api_id
...
carp(Dumper $params) if $self->debug>10;
  ...
  {    
    ...
    if ( keys %{ $method_discovery_struct } > 0 ) ## method discovery struct ok
    {
      ...
      push(@teapot_errors, "Client Credentials do not include required scope to access $params->{api_endpoint_id}") unless $self->has_scope_to_access_api_endpoint( $params->{api_endpoint_id} ) ;
      ...
      foreach my $meth_param_spec ( keys %{ $method_discovery_struct->{parameters} })
      {
        ...
        $params->{options}{$meth_param_spec} =  $method_discovery_struct->{parameters}{$meth_param_spec}{default}  
                     if ( (not defined $params->{options}{$meth_param_spec}  )
...
carp("DEBUG: $meth_param_spec is defined in param->{options}") if $self->debug>10;
  ...
  if ( @teapot_errors > 0 ) ## carp and include in 418 response body the teapot errors
  {
    ...
    return Mojo::Message::Response->new( content_type => 'text/plain', code=> 418, message => 'Teapot Error - Reqeust blocked before submitting to server with pre-query validation errors', body => join("\n", @teapot_errors) );
  }
  else
  {
    #carp Dumper $params;
...
sub has_scope_to_access_api_endpoint
{
  ...
  if ( keys ( my $method_spec = $self->extract_method_discovery_detail_from_api_spec( $api_ep )) >0  ) ## empty hash indicates failure
  {
    ...
    my $required_scope_count = 0; ## if the final count of scope constraints = 0 then we will assume permission is granted - this has proven necessary for the experimental Google My Business because scopes are not defined in the current discovery data as at 14/10/18
