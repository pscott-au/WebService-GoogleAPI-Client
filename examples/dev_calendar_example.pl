...
if (-e './gapi.json') { say "auth file exists" if $DEBUG > 1 }
...
say 'CHI Root cache folder: ' . $gapi_agent->discovery->chi->root_dir()
    if $DEBUG > 1;                                  ## cached content temporary directory
...
#print my $v1 = qq{$api_struct->{preferred} $api_struct->{name} $api_struct->{version} https://www.googleapis.com/discovery/v1/apis/$key \n};
      say my $v2
          = qq{$api_struct->{preferred} $api_struct->{name} $api_struct->{version} $api_struct->{discoveryRestUrl}};
    }
...
#my $r =  $gapi_agent->api_query(  api_endpoint_id => 'calendar.events.list', options => {  calendarId => 'primary', timeMin => $timeMin }); #'2018-06-03T10:00:00-07:00'} );
  ...
  if (1 == 1)    ##  insert new event into calendar
  {
    ...
    say Dumper $meths_by_id->{'calendar.events.insert'};
    exit;
    my $r_data = insert_new_event_into_calendar($gapi_agent, $demo_calendar_id)->json;
    if ($r_data->{kind} eq 'calendar#event') {
      say "Sleeping for 20 secs so you can check the calendar";
      sleep(20);
      say "Deleteing event with eventId = $r_data->{id}";
      my $r2 = $gapi_agent->api_query(
        api_endpoint_id => 'calendar.events.delete',
...
#$r =  $gapi_agent->api_query(  api_endpoint_id => 'calendar.calendars.insert', options => { summary => 'A GENERATED NEW CALENDAR' });
  ...
  {
    #say Dumper $meths_by_id->{'calendar.calendars.insert'};#exit;
    $r = $gapi_agent->api_query(api_endpoint_id => '', options => {});
  }
...
sub insert_new_event_into_calendar {
  ...
  my $r         = $gapi_client->api_query(
    api_endpoint_id => 'calendar.events.insert',
    options         => {
      calendarId => $calendar_id,
      start      => { dateTime => $formatter->format_datetime($start_dt) },
      end        => { dateTime => $formatter->format_datetime($finish_dt) },
      summary    => $summary
    }
  );
...
sub render_calendar_list {
  my ($data) = @_;
  croak("expected hashref data with key kind of 'calendar#calendarList'")
      unless $data->{kind} eq 'calendar#calendarList';
