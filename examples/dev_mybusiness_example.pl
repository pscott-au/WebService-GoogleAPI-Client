...
if (1 == 0)    ## allows augment with Google My Business API Definition - as at 14th October
{
  ...
  say "keys of api discovery hashref = "
      . join(',', sort keys(%{ WebService::GoogleAPI::Client::Discovery->new->discover_all() }));
...
# say $gapi_agent->api_query( api_endpoint_id => 'gmail.users.messages.list')->to_string;
  foreach my $meth (
    qw/mybusiness.accounts.list  /)    ##      -- FAILERS -  mybusiness.categories.list mybusiness.attributes.list
  {
