...
get '/api_detail' => sub {
  ...
  $c->render(
    json => {
      api     => $gapi_client->get_api_discovery_for_api_id($api_id),
...
get '/test' => sub {
  ...
  $c->render(
    template => 'index2',
    one      => 'one',
    fnar     => encode_json([ $gapi_client->list_of_available_google_api_ids() ])
  );
};
  ...
  $c->render(
    json => {
      ...
      'Google::reCAPTCHA - A simple lightweight implementation of Google\'s reCAPTCHA for perl' =>
          'DEPREACTED - Google Replaced with reCAPTCHA v3',
